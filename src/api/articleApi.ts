...
请严格按照以下 JSON 格式返回结果，其中 content 字段使用 Markdown 格式：
{
  "title": "文章标题",
  "content": "# 文章标题\n\n引言内容（200字左右）\n\n## 1. 小节1标题\n\n小节1内容\n\n## 2. 小节2标题\n\n小节2内容\n\n## 3. 总结\n\n总结性内容（150字左右）",
  ...
  try {
    ...
    return {
      ...
      content: parsedResult.content || generateFallbackContent(params),
 ...
 * 生成备用内容（当AI调用失败时使用）
 */
function generateFallbackContent(params: ArticleGenerateRequest): string {
  ...
  // 构建 Markdown 内容
  let markdownContent = `# ${params.topic}：${params.audience}的深度思考\n\n`;
  ...
  template.sections.forEach((sectionTitle, index) => {
    markdownContent += `## ${index + 1}. ${sectionTitle}\n\n`;
    
    let content = '';
    if (sectionTitle === '总结') {
      switch (params.style) {
        case 'professional':
          content = `综上所述，${params.topic}对${params.audience}具有重要的指导意义。我们应当以专业的态度对待这个问题，在实践中不断完善和优化我们的方法论。通过系统性的学习和应用，相信每个人都能在这个领域取得进步。`;
          break;
        case 'casual':
          content = `总的来说，${params.topic}这个话题还是挺有意思的。作为${params.audience}，我们可以在日常生活中多留意这些细节，说不定会有意想不到的收获呢。记住，生活本身就是最好的老师。`;
          break;
        case 'humorous':
          content = `最后想说的是，${params.topic}这事儿嘛，${params.audience}们大可不必太过紧张。毕竟生活本来就充满了惊喜和意外，保持一颗平常心最重要。笑着面对，一切都会变得简单起来。`;
          break;
        case 'inspirational':
          content = `愿每一个${params.audience}都能在${params.topic}的道路上找到属于自己的那片天空。记住，最美的风景永远在前方，最好的自己永远在路上。相信自己，你一定可以做到！`;
          break;
      }
    } else {
      switch (params.style) {
